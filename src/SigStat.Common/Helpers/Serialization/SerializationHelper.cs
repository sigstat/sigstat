...
namespace SigStat.Common.Helpers
{
    public class SerializationHelper
    {
        ...
        {
            T desirializedObject = JsonConvert.DeserializeObject<T>(s, settings);

            return desirializedObject;
        }
        public static T DeserializeFromFile<T>(string path) where T:new()
        {
            T desirializedObject = JsonConvert.DeserializeObject<T>(File.ReadAllText(path),settings);
        ...
        {
            File.WriteAllText(path, JsonConvert.SerializeObject(o, Formatting.Indented, settings));
        }
