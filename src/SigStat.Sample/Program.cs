...
namespace SigStat.Sample
{
    class Program
    {
        ...
        private static void signerSampleRateBasedVerifier()
        {
            ...
            {
                var resultsSheet4 = p.Workbook.Worksheets.Add("SignerDep");
                ...
                foreach (SequentialTransformPipeline pip in pipeline_)
                {
                    ...
                    // Loader.SignerFilter = s2 => s2.ID == "34";
                    ...
                    foreach (Signer s2 in signers)
                    {
                        ...
                        Sheet.Cells[1, 8].Value = "AER";
                        Sheet.Cells[1, 9].Value = "Threshold";
                    ...
                    for (s = 1; s <= 20; s = s + 1)
                    {
                        ...
                        benchmark.Verifier.Classifier = new DtwClassifier()
                        {
                        ...
                        benchmark.Sampler = new TestingSampler(5);
                        ...
                        foreach (Signer sig in signers)
                        {
                            ...
                            if (signers.SingleOrDefault(o => o.ID == sig.ID).bestFrr > result.SignerResults.SingleOrDefault(e => e.Signer == sig.ID).Frr && signers.SingleOrDefault(o => o.ID == sig.ID).bestAer != 0)
                            {

                                signers.Single(o => o.ID == sig.ID).bestFrr = result.SignerResults.Single(e => e.Signer == sig.ID).Frr;
                        ...
                        resultsSheet.Cells[1, 11].Value = "s";
                        resultsSheet.Cells[1, 12].Value = "AER";
                        resultsSheet.Cells[s + 1, 11].Value = s;
                        resultsSheet.Cells[s + 1, 12].Value = result.FinalResult.Aer;
                    ...
                    resultsSheet.Cells[1, 4].Value = "signer";
                    resultsSheet.Cells[1, 5].Value = "Best SamplingFrequency";
                    resultsSheet.Cells[1, 6].Value = "Best Aer";
                    int j = 1;
                    foreach (Signer signer in signers)
                    {
                        resultsSheet.Cells[j + 1, 4].Value = signer.ID;
                        resultsSheet.Cells[j + 1, 5].Value = signer.bestSampleRate;
                        resultsSheet.Cells[j + 1, 6].Value = signer.bestFrr;
                        j++;
                    }
                    //*******************************************************************************
                    //---------------------------------------------------------------------
                    ...
                    resultsSheet4.Cells[1, 4].Value = "signer";
                    resultsSheet4.Cells[1, 5].Value = "Aer";
                    int i = 0;
                    ...
                    foreach (Signer signer in signers)
                    {
                        ...
                        benchmark.Loader.EnumerateSigners(k => k.ID == signer.ID);
                        ...
                        var result = benchmark.Execute(true);
                        resultsSheet4.Cells[i + 1, 4].Value = signer.ID;
                        resultsSheet4.Cells[i + 1, 5].Value = result.SignerResults.Single(z => z.Signer == signer.ID).Aer;
                        i++;
                    }
                    // ===================================================================
                    ...
                    benchmark2.Loader = b.Loader;

                    //Loader2.SignerFilter = s2 => s2.ID == "34";

                    benchmark2.Verifier = new Verifier();
                    benchmark2.Verifier.Pipeline = pip;

                    benchmark2.Verifier.Classifier = new DtwClassifier()
                    {
                        DistanceFunction = Accord.Math.Distance.Euclidean,
                        // WarpingWindowLength = 2000,
                        //Sampler = new EvenNSampler(10),
                    ...
                    var Sheet2 = p.Workbook.Worksheets.Add("normalresults");
                    int s4 = 1;
                    foreach (Signer sig in signers)
                    {
                        ...
                        Sheet2.Cells[1, 8].Value = "AER";
                        Sheet2.Cells[1, 9].Value = "Threshold";
                    ...
                    p.SaveAs(new FileInfo($"C:/Users/Saleem/OneDrive/Mohammad/Ideas/samplerateDep/${database}_{str2}.xlsx"));
                    p = new ExcelPackage();
                }
            }
